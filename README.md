...
- Returns can happen early — submitting the return inspection immediately frees the car for any future dates regardless of the agreed end date. The return form also asks for the actual **return date** (defaults to today, can be backdated), which is used to show the admin the agreed rental cost (per the original dates) next to the actual cost (per the real return date) so they can see at a glance whether a refund or an extra charge is owed. Submitting the return inspection (by an inspector or an admin) does **not** by itself finalize the reservation — it stays `delivered` while the AI damage comparison runs, so a full admin can review the findings and click **Confirm Return** at `/admin-portal/reservations` (this is what actually moves it to `completed` and frees the car). Only then does the "Add a charge or refund" form appear, letting the admin log a `charge` (damage, late return) or a `refund` (early return, goodwill) as a `reservation_charges` row; refunds are netted out of the revenue figures in `/admin-portal/reports` (by the month the refund itself was issued, not the original booking month).
- Delivery and return inspections capture one photo per side of the car (front/back/left/right) instead of an unlabeled batch, so the two sets can be matched up angle-by-angle. When a return is submitted, each angle pair is sent to Gemini (if `GEMINI_API_KEY` is set, run in the background after the save so it never blocks or risks timing out the inspector's submit) asking it to describe any *new* damage visible in the return photo that wasn't in the delivery photo — for each distinct damage point it finds, it returns an English description, an Arabic translation, and a rough estimated EGP repair cost, stored as separate rows in `handover_damage_findings`. Shown as a flagged "AI damage check" note (in whichever language the viewer's site locale is) to both admins and inspectors as soon as it's ready, at `/admin-portal/reservations` and `/admin-portal/handovers` — for the admin to review before confirming the return and charging anything; it never auto-charges. The customer only sees the findings (at `/account/reservations`) after the admin confirms the return.
- A lightweight `inspector` role (`profiles.role = 'inspector'`, separate from `is_admin`) can sign in at `/admin-portal/login` but only sees the handover queue, customer contact info, and cars — not listings management, reservation approval, reports, or the users page. Full admins create Admin or Inspector accounts from `/admin-portal/users` (uses the Supabase service-role key server-side — the new account's session is separate from the admin's own). From that same page an admin can reset a team member's password or lock/unlock their account (Supabase Auth's `banned_until`, checked at sign-in — no app-side check needed) — an admin can't lock their own account.
