...
8. `supabase/seed.sql` — adds 3 sample rent listings and 3 sample sale listings (optional, for local dev)
...
- A lightweight `staff` role (`profiles.role = 'staff'`, separate from `is_admin`) can sign in at `/admin/login` but only sees the handover queue, customer contact info, and cars — not listings management, reservation approval, or ID documents. Promote a user with `update public.profiles set role = 'staff' where email = '...';` after creating their Supabase Auth account.
- Admins get in-app notifications (bell icon in the dashboard header) for reservation lifecycle events — new requests, approvals, documents submitted, payment recorded, confirmations, handovers, fines. Staff only see the handover-relevant slice.
- `/admin/reports` has charts (via `recharts`) for reservations-by-status and revenue-by-month, plus per-car and per-customer breakdowns.
